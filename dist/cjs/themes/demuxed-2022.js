...
var _breakpoints, _getBreakpoints, getBreakpoints_fn;
...
class MediaThemeDemuxed extends import_media_theme_element.MediaThemeElement {
  constructor() {
    super(...arguments);
    __privateAdd(this, _getBreakpoints);
    __privateAdd(this, _breakpoints, { xs: 360, sm: 600, md: 760, lg: 960, xl: 1100 });
  }
  ...
  connectedCallback() {
    this.render();
    const resizeObserver = new import_server_safe_globals.globalThis.ResizeObserver((entries) => {
      entries.forEach((entry) => {
        entry.target.className = __privateMethod(this, _getBreakpoints, getBreakpoints_fn).call(this, entry.contentRect).join(" ");
      });
    });
...
_breakpoints = new WeakMap();
_getBreakpoints = new WeakSet();
getBreakpoints_fn = function(rect) {
  return Object.keys(__privateGet(this, _breakpoints)).filter((key) => {
    return rect.width >= __privateGet(this, _breakpoints)[key];
  });
};
